...
#[query]
#[candid_method(query)]
pub fn get_file(data: String) -> Result<String, String> {
    ...
    for user in users {
        if user.address.to_string() == auther_id {
            let file_node: FileNode = user_files
                ...
                .expect("get_fil eNo file found in vertices")
                .clone();
            let tree = file_node
                .element_tree
                .and_then(|element_tree_id| element_trees.get(&user)?.get(&element_tree_id))
                .expect("get_file  No tree found in element_tree")
                .clone();
            if file_node.file_mode == FileMode::Public || current_user == user
            // || file_node.permeted_users.contains(&user)
            {
                return serde_json::to_string(&(file_node, tree)).map_err(|e| e.to_string());
            }
        }
    }
