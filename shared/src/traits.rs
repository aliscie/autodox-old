...
pub trait Store {
    ...
    async fn exec_create<T: Creatable, U>(&self, data: T) -> Result<U, Self::Error>;
    async fn exec_update<T: Updatable, U>(&self, data: T) -> Result<U, Self::Error>;
    async fn exec_select<T: Queryable, U>(&self, data: T) -> Result<U, Self::Error>;
}
