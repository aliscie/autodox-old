...
pub async fn get_directories() -> Result<Option<FileDirectory>, String> {
    ...
    if info.get().is_web || info.get().is_online {
        ...
        let file_tree: Result<Option<FileDirectory>, serde_wasm_bindgen::Error> =  serde_wasm_bindgen::from_value(response);
        return file_tree.map_err(|_e| "serde error".to_string());
    }
    if !info.get().is_web {
