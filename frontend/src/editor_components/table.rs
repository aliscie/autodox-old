...
use shared::id::Id;
use shared::schema::{EditorChange, EditorElement, ElementTree};
...
pub fn Table(props: &Props) -> Html {
    ...
    // TODO panicked at 'called `Option::unwrap()` on a `None` value', frontend/src/editor_components/table.rs:51:10
    ...
    // let children = global_state.element_tree.elements.adjacency.get(&table_id).unwrap_or(&Vec::new());
    ...
    //                         return html! {
    //                             <td style="border: 1px solid red" id = { table_cell.id.to_string()}> {table_cell.content.clone()}</td>
    //                         };
    //                     }
    //                     html! {}
    //                 }).collect::<Html>();
